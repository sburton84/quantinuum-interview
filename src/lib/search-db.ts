 ...
 */
export type SearchResultHit = { title: string; url: string; site: string, context: string | null };
...
export function searchAllIndicesFromDb(
  ...
  try {
    ...
    const titleQuery = connection.prepare(`
      SELECT t.title, ti.anchor, d.path, s.site_name
      FROM titles t
    ...
    const termQuery = connection.prepare(`
      SELECT t.term, ti.context, d.path, d.title, s.site_name
      FROM terms t
    ...
    const titleRows = titleQuery.all(ftsQuery) as { title: string; anchor: string | null; path: string; site_name: string }[];
    const termRows = termQuery.all(ftsQuery) as { term: string; context: string; path: string; title: string; site_name: string }[];
    ...
    for (const row of titleRows) {
      ...
      results.push({ title: row.title, url, site: row.site_name, context: null });
    }
    ...
    for (const row of termRows) {
      ...
      results.push({ title: row.title, url, site: row.site_name, context: row.context });
    }
    return results;
  } catch (e) {
    return [];
